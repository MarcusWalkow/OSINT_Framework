...
def add_info(project,knoten, name, zeitpunkt, gewicht, werkzeug):
    connection = sqlite3.connect(f"projects/{project}/database.db")
    cursor = connection.cursor()
    sql = """
            INSERT INTO graph(
            ...
            VALUES(?,?,?,?,?) 
            """
    werte=(name, knoten, zeitpunkt, gewicht, werkzeug)
...
def add_file(project,knoten, name, zeitpunkt, gewicht, werkzeug, pfad):
    connection = sqlite3.connect(f"projects/{project}/database.db")
    cursor = connection.cursor()
    sql = """
            INSERT INTO graph(
            ...
            VALUES(?,?,?,?,?,?) 
            """
    werte=(name, knoten, zeitpunkt, gewicht, werkzeug, pfad)
    cursor.execute(sql, werte)
    connection.commit()
    connection.close()
...
def del_entry(project,knoten,name):
    connection = sqlite3.connect(f"projects/{project}/database.db")
    cursor = connection.cursor()
    cursor.execute("""
        DELETE FROM table
        WHERE knoten = ? AND name = ?;""",
        (knoten,name)
    )
    cursor.close()
    connection.close()
    return f"Knoten {name} wurde gelöscht"
...
node ist der recherchierte Knoten
name ist der gefundene Wert
"""
def check_for_edge(project,node,name):
    connection = sqlite3.connect(f"projects/{project}/database.db")
    cursor = connection.cursor()
    cursor.execute("""
        SELECT rowid FROM graph 
        WHERE knoten=? and name=?""", (node,name)) 
    #durch das , hinter Name ist es ein Tuple, das nötig für die Abfrage ist, alternativ ginge auch [name]
    id = cursor.fetchone()
    cursor.close()
    connection.close()
    if (id is None):
